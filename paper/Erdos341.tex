...
\title{A Counterexample to Erd\H{o}s Problem~341}
\author{Zhiheng Li}
\date{}

\hypersetup{
  pdftitle={A Fixed Aperiodic Greedy Pair-Sum-Avoiding Sequence},
...
\begin{abstract}
We give a negative answer to Erd\H{o}s problem~341 by exhibiting the following counterexample
\[
A=\{1,2,3,5,7,13,22,27,28,32,36,40,47,48,52,63,71,77,81,89,97\}.
\]
The proof constructs an infinite set \(S\) satisfying the exact greedy recurrence \(t\in S\Longleftrightarrow t\notin S+S\) for \(t>97\). We show that \(S\) is the extension of the \(21\)-element seed set \(A\), and use a scale-eight sumset controller embedded in three residue classes modulo \(49\) and a finite modular shield to prove that the consecutive gap sequence of \(S\) is not eventually periodic.
\end{abstract}
...
\noindent\textit{Formal verification.} Theorem~\ref{thm:main} has been formally verified in Lean~4. The formalization is available at \href{https://doi.org/10.5281/zenodo.21902147}{https://doi.org/10.5281/zenodo.21902147}.
