...
\]
whose greedy pair-sum-avoiding extension has a consecutive-gap sequence that is not eventually periodic. The proof constructs an explicit infinite set \(S\) satisfying the exact greedy recurrence \(t\in S\Longleftrightarrow t\notin S+S\) for \(t>97\), using a scale-eight sumset controller embedded in three residue classes modulo \(49\) and a finite modular shield.
\end{abstract}
